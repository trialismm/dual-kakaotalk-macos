...
로그 폴더 권한은 `0700`, 로그 파일 권한은 `0600`입니다. 지원하지 않는 빌드 또는 설치 실패가 발생하면 로그를 표시하고 GitHub 이슈 페이지를 열 것인지 먼저 묻습니다. 로그는 자동으로 업로드되지 않습니다. 이슈에 붙여 넣기 전에 사용자 이름, 개인 경로 등 개인정보가 없는지 직접 확인하세요.
