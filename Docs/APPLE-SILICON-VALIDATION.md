...
**M3/macOS 26.5.2 검증 완료.** MacBook Air `Mac15,13`의 native arm64 환경에서 beta.16 설치·제거, 개인용 앱과 동시 실행 및 초록색 메뉴 막대 아이콘을 확인했습니다. 다른 Apple Silicon 칩과 macOS 조합은 여전히 실험적이며 아래 절차로 추가 검증합니다.
...
- 별도의 Dock 항목과 듀얼 앱의 `듀얼 카카오톡` 표시 이름
- 개인용 앱의 `카카오톡` 표시 이름
